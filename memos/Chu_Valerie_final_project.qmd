...
- I do think the reason the predictions are a bit higher than they actually are for the upper outliers is because the random forest model is not a parametric model. But it is still great at capturing the majority of the data, probably because its large number of decision trees are independent of each other and thus not overly influenced by how different the avocados were to each other. 
