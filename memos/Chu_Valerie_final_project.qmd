...
I will be using the following model types: null, OLS, lasso, ridge, boosted tree, k nearest neighbors, and random forest.
...
- The lasso model is a regularized regression, parametric model with a mixture of 1 that adds a penalty term to standard regression. I am using it as a model to compare other models against. I tuned the penalty for this model. 

- The ridge model a regularized regression, parametric model with a mixture of 0 that adds a penalty term to standard regression. I am using it as a model to compare other models against. I tuned the penalty for this model. 

- The boosted tree model is a non-parametric model where multiple sequential simple regression trees are combined into a stronger model, and each tree is trained on the residuals from the previous sequence of trees. All trees are then combined together using an additive model, whose weights are estimated via the gradient descent procedure. I am using it as a model to compare other models against. I tuned the mtry, min_n, and learn_rate for this model. 

- The k nearest neighbors model is a non-parametric model that uses the K most similar data points from the training set to predict new samples. I am using it as a model to compare other models against. I tuned the neighbors for this model. 

- The random forest model is a non-parametric model that creates a large number of decision trees, each independent of the others. The final prediction uses all predictions from the individual trees and combines them, making this model type resistant to outliers. I am using it as a model to compare other models against. I tuned the mtry and min_n for this model. 
